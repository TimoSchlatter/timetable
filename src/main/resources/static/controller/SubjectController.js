...
app.controller('SubjectController', function ($scope, $http, $filter) {
    ...
    var setSelectableSubjects = function () {
        var subjectTypes = $filter('unique')($scope.subjects, 'subjectType');
        subjectTypes.unshift({subjectType: "Alle"});
        $scope.selectableSubjects = subjectTypes;
        $scope.selectedSubjectType = subjectTypes[0];
    };

    var getData = function () {
        $http.get(subjectsUrl).then(function successCallback(response) {
            $scope.subjects = response.data;
            setSelectableSubjects();
